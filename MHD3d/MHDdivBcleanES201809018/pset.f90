...
!--- temporary debug
 lfff_k = lfff_kly_pi; lfff_kk = lfff_k
...
!              & + xl*0.5d0
         tmpxvalb = 1d0
         do tmpxbeki = 1, nbeki
            tmpxvalb = tmpxvalb * (i+nx*index_x-nx3d/2)
         end do
         xc(i) = alphax*(i+nx*index_x-nx3d/2) + betax*tmpxvalb
...
!              & + yl * 0.5d0
         tmpyvalb = 1d0
         do tmpybeki = 1, nbeki
            tmpyvalb = tmpyvalb * (j+ny*index_y-ny3d/2)
         end do
         yc(j) = alphay*(j+ny*index_y-ny3d/2) + betay*tmpyvalb
...
implicit none
real(DP) :: fraction_fine_grid, delz, dzmin, dzmax, dzchg, bunbo
real(DP) :: zk0
integer :: k
