...
def get_tmpnx(num_inj):
    array_footedge = np.genfromtxt("../r3dbline/file_output/picture/foot/foot_edge.dat")
    tmp_footedge = np.max(array_footedge[:num_inj,0])
    return tmp_footedge
time_array_num = np.sum(time<float(infos[10]))
tmpnx = int(get_tmpnx(time_array_num) - xcut)
...
#    for i in range(0,nnx):
    for i in range(xcut-tmpnx,xcut+tmpnx+1):
        for k in range(0,nnz):
            #
            segxz[i,k] = dx[i]*dz[k]
...
np.savetxt("kappa_t"+str(timeinit)+".dat",
           [time[:],kappa[:]])
np.savetxt("PhitotBy.dat",Phi_tot)
np.savetxt("PhitotBz.dat",Phi_tot_test)
