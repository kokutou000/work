...
real(8),allocatable :: recflux(:)
integer :: ts, i, j, tsini, tsfin
integer :: nx,ny,nz,nxmin,nxmax,nymin,nymax,nzmin,nzmax
...
close(100)
nx = nxmax - nxmin; ny = nymax - nymin; nz = nzmax - nzmin
...
allocate(Bz(0:nx,0:ny,0:nz)); Bz=0
allocate(xc(0:nx)); xc=0
allocate(yc(0:ny)); yc=0
allocate(recflux(tsini:tsfin)); recflux=0

!--- open seed file
!
!open(21,file="seedall.txt",status="replace")

!--- open coordinate file to calculate reconnected flux
open(41,file="../../../../3dbin/coord.xgc",status="old")
read(41,'(e25.16)') xc
close(41)
open(42,file="../../../../3dbin/coord.ygc",status="old")
read(42,'(e25.16)') yc
close(42)
...
!------ and integrate reconnected flux from Bz on z=0
!
do ts = tsini+1, tsfin
write(cts,'(I3.3)') ts
write(*,*) "cts =", cts

open(33,file="../../../../3dbin/Bzbin_3d_R."//cts,form="unformatted",status="old")
read(33) Bz
close(33)
...
!      write(21,'(f10.5,2x,f10.5,2x,f10.5,3x,i4)') inifx(i,j), inify(i,j), inifz(i,j)+z_aj, ts-tsini
      recflux(ts) = recflux(ts) + abs(Bz(i,j,0))&
           &*(xc(i+1)-xc(i-1))*0.5d0&
           &*(yc(j+1)-yc(j-1))*0.5d0
!   end if
   endif
...
open(60,file="recflux.txt")
do ts = tsini, tsfin
   write(60,*) ts, recflux(ts)
end do
close(60)
